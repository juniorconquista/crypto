...
describe("GIVEN Crypto", () => {
  describe("WHEN rendered", () => {
    it("THEN should decrypt data using a private key", async () => {
      ...
      const sut = makeSut();
      const encryptedData = await sut.encrypt(publicKeyPEM, data);
      const decryptedData = await sut.decrypt(privateKeyPEM, encryptedData);
    ...
    it('AND should throw error on invalid encrypted data', async () => {
      const sut = makeSut();
      await expect(sut.decrypt(privateKeyPEM, 'invalidEncryptedData')).rejects.toThrow();
    });
  });
});
